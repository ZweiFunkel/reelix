...
**Nachträge (nach erstem echten Server-Deploy, 2026-07-25) — verifiziert:**
- Admin-Passwort-Reset per CLI: `docker exec <container> reelix-server reset-password <username> <neues-passwort>` — für den Fall "Passwort vergessen, keine Email hinterlegt/keine Session mehr". Lokal getestet (altes Passwort danach ungültig, neues funktioniert).
- Email + Self-Service-Passwort-Reset: `email`-Spalte am User, `PATCH /api/auth/me` zum Hinterlegen, "Forgot password?" auf der Login-Seite → 6-stelliger Code per Mail (SMTP über `REELIX_SMTP_*`-Env-Vars; ohne SMTP-Konfiguration wird der Code stattdessen ins Server-Log geschrieben, damit der Flow auch ohne Mail-Setup nicht komplett blockiert) → Code+neues Passwort → alle Sessions des Accounts werden invalidiert. Ende-zu-Ende getestet inkl. falscher Code (abgelehnt) und Session-Invalidierung.
- Player-OSD überarbeitet (bisher nur nackter `<video controls>`): eigene Steuerleiste mit Play/Pause, ±10s Skip, Lautstärke, Fullscreen; links der Fortschrittsleiste Restzeit, rechts Gesamtlänge + "Ends HH:MM" (Uhrzeit, wann der Titel fertig ist) — im Browser mit echtem 30s-Testvideo verifiziert (Skip korrekt geklemmt, Restzeit/Uhrzeit-Anzeige korrekt). Live-Kanäle zeigen stattdessen nur ein LIVE-Badge (kein Seek, da nicht sinnvoll).
- Username-Reset: `reelix-server rename-user <alt> <neu>` (CLI) + `PATCH /api/auth/me {username}` (Self-Service, eingeloggt) — Eindeutigkeit wird geprüft (freundlicher 409-Fehler statt rohem DB-Constraint-Error), zusätzlich `reelix-server list-users` zum Finden des echten Usernamens bei Verwechslung (kein sqlite3-Client nötig). Neuer Username sofort einsatzbereit fürs Login, alter sofort ungültig — verifiziert.
- **Pflicht-Email-Verifizierung beim ersten Login** (User-Wunsch, "so wie manche das machen"): `email_verified`-Flag am User; solange nicht verifiziert, blockiert die App komplett vor Profilauswahl — erst Email eintragen (falls noch keine hinterlegt), dann 6-stelligen Code eingeben. Selbe Code-Infrastruktur wie Passwort-Reset (`purpose`-Spalte unterscheidet `password_reset` vs. `email_verify` in derselben Tabelle). Email-Änderung setzt `email_verified` automatisch zurück (neue Adresse = neue Verifizierung nötig). Absender-Default jetzt `noreply@reelix.com` (per `REELIX_SMTP_FROM` überschreibbar — **wichtig:** die eigene SMTP-Domain nutzen, sonst blocken viele Provider das Absender-Mismatch). Ende-zu-Ende verifiziert: Setup ohne Email → Gate blockt → Email setzen → Code (im Log, da kein SMTP konfiguriert) → falscher Code abgelehnt → richtiger Code → verifiziert, App freigegeben.
